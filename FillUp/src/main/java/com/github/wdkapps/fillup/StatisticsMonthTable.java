...
public class StatisticsMonthTable implements HtmlData {
    ...
    private void appendMileageData() {
        ...
        value = "-";
        if (count > 0)
            value = String.format(App.getLocale(), "%.2f %s", sum / count, units.getMileageLabel());
        appendTableRow(new String[]{label, value});
        ...
        value = "-";
        if (count > 0)
            value = String.format(App.getLocale(), "%.2f %s", min, units.getMileageLabel());
        appendTableRow(new String[]{label, value});
        ...
        value = "-";
        if (count > 0)
            value = String.format(App.getLocale(), "%.2f %s", max, units.getMileageLabel());
        appendTableRow(new String[]{label, value});
    }
